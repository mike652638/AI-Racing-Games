...
- [x] **Step 1: 写失败测试** `tests/unit/bot.test.ts`
...
- [x] **Step 2: 运行验证失败**：`npx vitest run tests/unit/bot.test.ts` → FAIL（模块不存在）

- [x] **Step 3: 最小实现** `src/ai/bot.ts`
...
- [x] **Step 4: 运行验证通过**：`npx vitest run tests/unit/bot.test.ts` → PASS 7/7
...
- [x] **Step 1: 写失败测试** `tests/unit/simulate.test.ts`
...
- [x] **Step 2: 运行验证失败**：`npx vitest run tests/unit/simulate.test.ts` → FAIL（simulate 不存在、updateCar 无返回值）

- [x] **Step 3: 修改 `src/physics/car.ts`（updateCar 返回出界标记，其余逻辑不变）**
...
- [x] **Step 4: 实现 `src/ai/simulate.ts`**
...
- [x] **Step 5: 运行验证通过**：`npx vitest run tests/unit/simulate.test.ts` → PASS 8/8；再全量 `npm test` 确认旧 39 测试不回归（updateCar 返回值不破坏调用方）
...
- [x] **Step 1: `src/engine/track.ts` 追加**
...
- [x] **Step 2: `src/main.ts` 删掉内联赛道数组，改为**
...
- [x] **Step 3: 创建 `tests/bot/run-bot.ts`**
...
- [x] **Step 4: `package.json` scripts 加**

```json
"bot": "node tests/bot/run-bot.ts"
```

- [x] **Step 5: 验证**：先 `node tests/bot/run-bot.ts` 直接跑确认 node v25 原生 TS 可用（若报语法错误则改 `node --experimental-strip-types tests/bot/run-bot.ts`）；再 `npm run bot`；最后全量 `npm run typecheck && npm run lint && npm test && npm run build`
...
- [x] **Step 1: 确认 dev server（localhost:5173）存活**，agent-browser 打开页面；`keydown ArrowUp` 持续 5 秒后 `getImageData` 采样确认画面滚动正常（赛道改为默认赛道后渲染无回归）

- [x] **Step 2: `npm run typecheck && npm run lint && npm test && npm run build && npm run bot` 全绿**；`git status` 干净

- [x] **Step 3: 记录 M4 圈速基线**（run-bot 输出 lapTimes 与 violations，供 M5 结算目标时间参考）
...
- [x] **Step 1: 写失败测试** `tests/unit/format.test.ts`
...
- [x] **Step 2: 运行验证失败** → FAIL（模块不存在）

- [x] **Step 3: 最小实现 `src/ui/format.ts`**
...
- [x] **Step 4: 运行验证通过**：`npx vitest run tests/unit/format.test.ts` → PASS 6/6

- [x] **Step 5: `index.html` 加 HUD 覆盖层**（`#hud` 放 canvas 之上，绝对定位）
...
- [x] **Step 6: `src/style.css` 追加**（字体等宽、右上角布局、半透明底、mix-blend 可选；`#hud` 定位 absolute + `pointer-events: none`）

- [x] **Step 7: `src/main.ts` 集成**：每帧 `hudSpeed.textContent = formatSpeed(carState.speed, carConfig.maxSpeed)`、`hudLap.textContent = formatLap(lapFromZ(cameraZ, lapLength), TOTAL_LAPS)`、`hudTime.textContent = formatTime(cameraZ / carConfig.maxSpeed)`（注意：计时用累计行驶时间需另加 `raceTime += dt` 变量，HUD 显示 raceTime）；常量 `const TOTAL_LAPS = 3`、`const lapLength = track.length * SEGMENT_LENGTH`

- [x] **Step 8: 运行验证**：`npm run typecheck && npm run lint && npm test`；浏览器实测 HUD 数字随加速变化
...
- [x] **Step 1: 写失败测试** `tests/unit/engine-audio.test.ts`
...
- [x] **Step 3: 最小实现 `src/audio/engine.ts`**
...
- [x] **Step 4: 运行验证通过**：`npx vitest run tests/unit/engine-audio.test.ts` → PASS 4/4

- [x] **Step 5: `src/main.ts` 接入**：首次按键时 `engineSound = new EngineSound(new AudioContext())` + `engineSound.start()`（autoplay 策略）；每帧 `engineSound.setSpeedRatio(carState.speed / carConfig.maxSpeed)`

- [x] **Step 6: 运行验证**：`npm run typecheck && npm run lint && npm test`；浏览器实测 `eval "new AudioContext().state"` → running、console 无报错
...
- [x] **Step 1: 写失败测试** `tests/unit/gamestate.test.ts`
...
- [x] **Step 2: 运行验证失败** → FAIL

- [x] **Step 3: 最小实现 `src/ui/gamestate.ts`**
...
- [x] **Step 4: 运行验证通过**：PASS 4/4

- [x] **Step 5: `index.html` 加覆盖层**：`#start-screen`（标题 `OUTRUN 伪3D` + 操作提示 + “按任意键开始”）、`#finish-screen`（hidden，含 #finish-time #finish-lap 文本 + “按 R 重新开始”）；keydown 处理：menu → racing（并启动音效）；racing 中 R 键不动；finished → R 键回 menu

- [x] **Step 6: `src/main.ts` 状态机**：`phase` 变量 + `raceTime`；racing 时每帧 `raceTime += dt`、`lap = lapFromZ(cameraZ, lapLength)`、`if (lap > TOTAL_LAPS) phase = PHASE_FINISHED`（显示结算：总用时 formatTime(raceTime)、平均速度）；菜单态不更新物理只渲染背景；finished 态冻结画面

- [x] **Step 7: `src/style.css`**：覆盖层全屏居中、大标题渐变字、半透明底

- [x] **Step 8: 运行验证**：`npm run typecheck && npm run lint && npm test && npm run build`；浏览器实测：初始见启动画面 → keydown 后进入比赛、HUD 显示 → 加速后音效可听（state=running）→（可选快速验证：改 TOTAL_LAPS=1 临时确认结算出现，测完还原）
...
- [x] **Step 1: 写 `README.md`**：项目简介（OutRun 伪3D 复刻、技术栈）、快速开始（npm install / npm run dev / npm run build / npm run preview）、测试（npm test、npm run bot 及报告格式示例）、操作说明（WASD/方向键、任意键开始、R 重开）、里程碑状态表（M1-M5 全部 ✅）、目录结构、bot 圈速基线（引用 Task 4 记录）

- [x] **Step 2: 最终验收**：`npm run typecheck && npm run lint && npm test && npm run build && npm run bot` 全部通过；浏览器全流程实测一遍（启动画面→比赛→HUD/音效→3 圈结算）；`git status` 干净
