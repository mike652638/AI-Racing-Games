...
M16 起演进档案形成"计划 + 研究报告"双轨：计划在前（可执行 Task 契约），报告在后（实测取证 + 交付结论）。本目录 18 份核心报告按主题分为六类——项目级复盘（5 份）、运行时实测与修复（4 份）、工程化改进（1 份）、视觉/渲染优化（2 份）、环境差异化（4 份）、菜单 UI/UX 优化（2 份）；其余 M18-M20 批次报告散列于目录，见文件列表。
...
5. **M20 菜单布局优化闭环（2026-08-06，2 份）**：`menu_track_preview_layout.md` 诊断 1920×1080 大屏赛道预览区纵向挤压 → 实施「SVG 背景化 + 精简内容区」方案（`buildTrackPreviewBackgroundSvg` 全屏背景层 + 小屏中央预览降级 + 融合度两轮优化）→ `start_screen_responsive_audit.md` 以 17 视口矩阵自动化实测（agent-browser 串行视口 + @observer 视觉复核）发现移动横屏榜单隐形、桌面低高度被裁等 P0-P3 共 11 项问题，全部修复（新增 `max-height:800px` 低高度桌面分支、移动横屏默认收起榜单 + 渐隐滚动提示、竖屏 rotate-hint 上移）并归档修复记录。
...
| `research_report_menu_track_preview_layout.md`     | 菜单「赛道预览」区域布局优化建议 + 实施记录：宽屏全屏 SVG 背景层（buildTrackPreviewBackgroundSvg 复用控制点积分与环境配色）替代中央预览 + 小屏降级 + 融合度两轮优化（714 → 716 用例全绿） |
| `research_report_start_screen_responsive_audit.md` | #start-screen 全视口响应式审计与修复：17 视口矩阵实测发现 P0-P3 共 11 项（移动横屏榜单隐形/桌面低高度被裁/竖屏提示遮挡等），全部修复 + 独立复核验证数据（716 用例 + 6 视口零溢出）        |
| `research_report_project_analysis_2026-08-06.md`   | 项目深入分析报告（M20 状态，2026-08-06 实测）：架构分层、依赖方向、测试门禁与遗留风险（game-loop.ts 超大类等）快照                                                                        |
