...
| `tests/`       | 质量验证层：64 个 Vitest 测试文件 998 用例（unit 63 文件 997 用例 + bench 冒烟 1；含 canvas mock 基建与 tests/helpers 测试辅助）+ Playwright 视觉回归（M16，tests/e2e 桌面/移动/大屏三 project，M18 起含玩法链路 BOOST/碰撞反馈断言，M20 起含大屏 1920×1080 project）+ bot 跑圈验收脚本（9 赛道矩阵回归）+ 性能基准与安全扫描工具（tests/bench）                                            | [tests/codemap.md](tests/codemap.md)             |
