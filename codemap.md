...
| `src/game/game-loop.ts` | 主循环编排壳（1133 行，2026-08-15 M34 实测；M22-M33 可玩性提升与实测修复批次后曾达 1490 行，M34 第二轮瘦身 1490→1133）：DOM 组装（dom-setup）、每帧更新/渲染薄壳（纯函数下沉 frame-update/frame-render/frame-pure）、四模式策略（mode-strategy）、结算记账（finish-accounting）、双玩家赛道管理、音频装配（audio-rig）、调试钩子；M34 新增 6 个下沉模块（game-params/menu-setup/route-choice/boost-feedback/track-cards/pause-controls） |
...
| `src/game/`    | 游戏编排层：GameLoop 编排壳 + 纯函数下沉件（mode-strategy 策略对象/frame-update/frame-render/frame-pure/finish-accounting/collision-feedback 红闪状态机/audio-rig/countdown/dom-setup/track-preview，M34 起 + game-params/menu-setup/route-choice/boost-feedback/track-cards/pause-controls）、双玩家 TrackContext 赛道世界、RaceState、阶段 FSM（直接导入 shared）、挑战模式/热座/分屏支持 | [src/game/codemap.md](src/game/codemap.md)       |
...
| `tests/`       | 质量验证层：65 个 Vitest 测试文件 1026 用例（unit 64 文件 1025 用例 + bench 冒烟 1；含 canvas mock 基建与 tests/helpers 测试辅助）+ Playwright 视觉回归（M16，tests/e2e 桌面/移动/大屏三 project，M18 起含玩法链路 BOOST/碰撞反馈断言，M20 起含大屏 1920×1080 project）+ bot 跑圈验收脚本（9 赛道矩阵回归）+ 性能基准与安全扫描工具（tests/bench）                                          | [tests/codemap.md](tests/codemap.md)             |
...
- 阶段管理采用有限状态机（`menu` / `racing` / `paused` / `finished`），`Phase` 定义与转移逻辑唯一真源在 `src/shared/phase.ts` / `src/shared/phase-logic.ts`（`ui/gamestate.ts` re-export 兼容层已于 2026-08-05 删除，hud/screens 直接导入 shared，game 层亦直接导入 shared）。
...
- **M35 QA 实测修复与打磨批次（2026-08-22）**：`#daily-badge` 每日挑战徽章机制——frame-update 按 `isDailyTrack` 输入显隐、game-loop startGame 计算（`!routeMode && dailyModeEnabled && 当前赛道 === rollDailyToToday(loadDaily()).trackId`，路线模式保守隐藏；金色胶囊徽章样式 `--color-accent` 令牌）；`GameLoop.destroy()` 幂等生命周期——destroyed 标志 + frame 首行守卫防旧帧自续，统一清理 RAF/window 监听/倒计时/持续音/音频装备束/joystick/touchQuadrant（监听/定时器经清理函数登记，S3）；夜间车流双尾灯收敛参数——核心灯 0.15w×0.07h 镜像对称（原 0.2w×0.25h 且右灯贴边不对称，合计约 10% 车身面积遮挡车身）+ 外层 rgba(255,59,48,0.25) 光晕层（合计约 5%，保留夜间点亮感）；雨滴确定性抖动——`RAIN_DROPS_MULT` ×1.4 密度（80→112）+ `RAIN_ANGLE_JITTER_DEG` ±4° 每滴 angleOffset（mulberry32(2026) 确定性生成）。
