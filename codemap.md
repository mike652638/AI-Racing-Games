...
- 游戏参数唯一真源为 `src/shared/constants.ts`（2026-08-05 由 game/constants 提升至独立共享层），engine/physics/ui 直接导入常量避免魔法数字；车流密度经 `TrackDef.trafficCount` 赛道级调参（缺省 `TRAFFIC_DEFAULT_COUNT = 14`），天气三态循环（晴/阴/雨各 45s）由 `updateLighting` 参数化。
- **依赖方向（2026-08-05 已解环 game↔ui）**：game→ui 为运行级调用（hud/screens/joystick/save/minimap）；ui→game 仅剩 `import type`（hud/screens 的 `RaceState`、hud/minimap 的 `TrackContext`，编译期擦除）；ui→game 的运行时依赖（`DRIFT_SCORE_MAX`/`CHALLENGE_TARGET_SCORE` 与 `gamestate`/`format` 的 re-export）已全部改从独立共享层 `src/shared/` 导入（constants/phase/phase-logic/lap 唯一真源），engine/physics 亦直接依赖 `src/shared/constants`——运行时依赖环已消除，无循环初始化问题；game 层保留 re-export 兼容层。
