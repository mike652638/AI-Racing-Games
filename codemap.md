...
| `src/game/game-loop.ts` | 主循环编排壳（1200 行，2026-08-05 wc -l 实测）：DOM 组装（dom-setup）、每帧更新/渲染薄壳（纯函数下沉 frame-update/frame-render/frame-pure）、四模式策略（mode-strategy）、结算记账（finish-accounting）、双玩家赛道管理、音频装配（audio-rig）、调试钩子 |
...
| `tests/`       | 质量验证层：49 个 Vitest 测试文件 707 用例（unit 48 + bench 冒烟 1；含 canvas mock 基建与 tests/helpers 测试辅助）+ Playwright 视觉回归（M16，tests/e2e 桌面/移动双 project，M18 起含玩法链路 BOOST/碰撞反馈断言）+ bot 跑圈验收脚本（9 赛道矩阵回归）+ 性能基准与安全扫描工具（tests/bench）                                      | [tests/codemap.md](tests/codemap.md)             |
