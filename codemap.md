...
| `src/game/game-loop.ts` | 主循环编排壳（927 行）：DOM 组装（dom-setup）、每帧更新/渲染薄壳（纯函数下沉 frame-update/frame-render/frame-pure）、四模式策略（mode-strategy）、结算记账（finish-accounting）、双玩家赛道管理、音频装配（audio-rig）、调试钩子                                          |
...
| 目录           | 职责摘要                                                                                                                                                                     | 详细地图                                         |
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| `src/engine/`  | 伪 3D 渲染引擎层（Canvas 2D）：投影数学、赛道定义与生成、路面几何/曲面（road-surface）、景物系统（sprite-draw 形状/terrain-draw 地形装饰）、车流系统（traffic-draw）、环境配置（environment.ts 9 种环境）、玩家车渲染（player-car）、屏幕特效（screen-effects）、road-strip 离屏缓存、光照、RenderView 多 view 参数化、确定性 PRNG | [src/engine/codemap.md](src/engine/codemap.md)   |
| `src/shared/`  | 独立共享层（2026-08-05 解环新建）：常量唯一真源 constants.ts、阶段 phase/phase-logic、圈数 lap、共享类型 types（RaceState/TrackContext 类型提升）；engine/physics/ui 直接导入，运行时依赖图最底层，零运行时依赖 | [src/shared/codemap.md](src/shared/codemap.md)   |
...
| `tests/`       | 质量验证层：47 个 Vitest 单元测试 681 用例（含 canvas mock 基建与 tests/helpers 测试辅助）+ Playwright 视觉回归（M16，tests/e2e 桌面/移动双 project）+ bot 跑圈验收脚本（9 赛道矩阵回归）                                          | [tests/codemap.md](tests/codemap.md)             |
...
- **依赖方向（2026-08-05 已解环 game↔ui，含类型层）**：game→ui 为运行级调用（hud/screens/joystick/save/minimap）；ui→game 已无任何依赖——原仅剩的 `import type`（RaceState/TrackContext）已提升至 `src/shared/types.ts`（仅 import type 转发，编译期擦除），ui 改从 shared 导入；ui→game 的运行时依赖（`DRIFT_SCORE_MAX`/`CHALLENGE_TARGET_SCORE` 与 `gamestate`/`format` 的 re-export）已全部改从独立共享层 `src/shared/` 导入（constants/phase/phase-logic/lap/types 唯一真源），engine/physics 亦直接依赖 `src/shared/constants`——运行时与类型层依赖环均已消除，ui → game 100% 解耦；game 层保留 re-export 兼容层。
