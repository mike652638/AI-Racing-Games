...
| `tests/`       | 质量验证层：47 个 Vitest 单元测试 685 用例（含 canvas mock 基建与 tests/helpers 测试辅助）+ Playwright 视觉回归（M16，tests/e2e 桌面/移动双 project，M18 起含玩法链路 BOOST/碰撞反馈断言）+ bot 跑圈验收脚本（9 赛道矩阵回归）                                          | [tests/codemap.md](tests/codemap.md)             |
