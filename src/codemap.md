...
- **依赖方向**：`engine/` 与 `physics/` 处于底层；`game/` 依赖 `engine/` + `physics/`，并对 `ui/` 存在运行级调用（hud/screens/joystick/save/minimap）；`ui/` 对 `game/` 仅剩 `import type`（hud/screens 的 `RaceState`、hud/minimap 的 `TrackContext`，编译期擦除）；常量唯一真源已提升至独立共享层 `src/shared/`（constants/phase/phase-logic/lap，**2026-08-05 解环 game↔ui**）——ui 的 `DRIFT_SCORE_MAX`/`CHALLENGE_TARGET_SCORE` 与 gamestate/format 的 re-export 均改从 shared 导入，engine/physics 亦直接依赖 `src/shared/constants`，运行时依赖环已消除（game 层 constants/phase/phase-logic/lap 保留 re-export 兼容层）；`audio/` 仅依赖外部注入的 `AudioContext`；`ai/` 依赖 `engine/` + `physics/`。
