...
- **依赖方向**：`engine/` 与 `physics/` 处于底层；`game/` 依赖 `engine/` + `physics/`，并对 `ui/` 存在运行级调用（hud/screens/joystick/save/minimap）；`ui/` 对 `game/` 已完全断开——连 `import type`（RaceState/TrackContext）也已提升至 `src/shared/types.ts`（2026-08-05 解环，ui 从 shared 导入类型），运行时依赖（`DRIFT_SCORE_MAX`/`CHALLENGE_TARGET_SCORE` 与 gamestate/format 的 re-export）全部改从 `src/shared/` 导入（constants/phase/phase-logic/lap/types 唯一真源），engine/physics 亦直接依赖 `src/shared/constants`——运行时与类型层依赖环均已消除（game 层 constants/phase/phase-logic/lap/state/track-context 保留 re-export 兼容层）；`audio/` 仅依赖外部注入的 `AudioContext`；`ai/` 依赖 `engine/` + `physics/`。
...
| `src/style.css`     | 全屏 Canvas、HUD 定位（含分屏左右对称布局）、启动/暂停/结算画面样式（9c57c8f 起 `#finish-screen` 加 `overflow-y: auto` 防内容溢出截断；2026-08-05 起 `hideIfEmpty` 空边框卡片修复）、赛道选单双类高亮（selected/selected-p2）+ 赛道缩略图放大与预览光晕（74742d8）、菜单与 HUD 细节打磨（暂停按钮可点性）、移动端适配；M18 起新增颜色令牌（`--color-p1/--color-p2/--color-danger/--color-accent` + RGB 变量）、`prefers-reduced-motion` 降级区块、`.leaving/.pause-in` 屏幕过渡、`.no-charge` BOOST 红闪、`.btn-primary` 公共按钮、集中 `focus-visible` 焦点环、`#racing-touch-hint` 触屏引导浮层、自定义滚动条与低高度移动端压缩 |
