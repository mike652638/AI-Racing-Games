...
- **依赖方向**：`engine/` 与 `physics/` 处于底层；`game/` 依赖 `engine/` + `physics/`，并对 `ui/` 存在运行级调用（hud/screens/joystick/save/minimap）；`ui/` 对 `game/` 已完全断开——连 `import type`（RaceState/TrackContext/PlayerState）也已提升至 `src/shared/types.ts`（2026-08-05 RaceState/TrackContext、2026-08-15 PlayerState 迁入，ui 从 shared 导入类型），运行时依赖（`DRIFT_SCORE_MAX`/`CHALLENGE_TARGET_SCORE` 与 format 的 re-export）全部改从 `src/shared/` 导入（constants/phase/phase-logic/lap/types 唯一真源；ui/gamestate 纯 re-export 兼容层 2026-08-05 删除，hud/screens 直接导入 shared/phase），engine/physics 亦直接依赖 `src/shared/constants`——运行时与类型层依赖环均已消除（game 层 re-export 兼容层已于 2026-08-22 移除，state/track-context/player-state 仅保留工厂函数，类型经 import type 自 shared/types 引入）；`audio/` 仅依赖外部注入的 `AudioContext`；`ai/` 依赖 `engine/` + `physics/`。
...
- **可玩性提升系列（M22-M32）**：`game/achievements.ts`（8 项成就判定）、`game/daily.ts`（每日挑战日期确定性轮换）、`game/near-miss.ts`（贴身超车判定/蓄能）、`shared/medal.ts`（S/A/B 奖牌）、`ui/touch-quadrant.ts`（分屏四分区触控）、`engine/guide-line.ts`（M28 导航引导线 + 岔路分叉带渲染）、`engine/routes.ts`（OutRun 式岔路路线数据结构）；入口 `?route=1|2|3` / `?guide=1` / `?daily=1` / `?weather=` / `?traffic=`。
- **M34 game-loop 第二轮瘦身（2026-08-15，1490→1133 行）**：`game/game-params.ts`（URL 模式解析纯函数）、`game/menu-setup.ts`（菜单静态装饰）、`game/route-choice.ts`（路线模式集中化——原散落五处的段末检测/岔路选择/分支切换收敛为单一真源）、`game/boost-feedback.ts`（BOOST 未蓄能红闪纯状态机）、`game/track-cards.ts`（赛道卡构建 + 网格导航）、`game/pause-controls.ts`（暂停控件注入式绑定）；`shared/timer.ts`（unrefSafeTimeout 收敛两处重复计时）；`ui/screens.ts` 增 `applyPauseBranding`（暂停标题按玩家标注）/`revealRacingTouchHint`；`game/top-refresh.ts` 增 `refreshMenuBoard`（两处菜单刷新消重）。
