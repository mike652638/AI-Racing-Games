...
- **依赖方向**：`engine/` 与 `physics/` 处于底层；`game/` 依赖 `engine/` + `physics/`，并对 `ui/` 存在运行级调用（hud/screens/joystick/save/minimap）；`ui/` 对 `game/` 已无任何依赖（2026-08-05 类型提升：原仅剩的 `import type`（RaceState/TrackContext）已提升至 `src/shared/types.ts`，ui 改从 shared 导入）；常量唯一真源已提升至独立共享层 `src/shared/`（constants/phase/phase-logic/lap/types，**2026-08-05 解环 game↔ui（含类型层，100% 解耦）**）——ui 的 `DRIFT_SCORE_MAX`/`CHALLENGE_TARGET_SCORE` 与 gamestate/format 的 re-export 均改从 shared 导入，engine/physics 亦直接依赖 `src/shared/constants`，运行时与类型层依赖环均已消除（game 层 constants/phase/phase-logic/lap 保留 re-export 兼容层）；`audio/` 仅依赖外部注入的 `AudioContext`；`ai/` 依赖 `engine/` + `physics/`。
...
- **M17 环境差异化**：`engine/environment.ts`（9 种环境配置唯一真源：天空/草地色相、远山配色、景物类型与密度、地形装饰）；`TrackDef.environment` 字段；渲染层按环境差异化光照/远山/景物形状（SpriteKind 扩展 cactus/palm/snowpile + rotation/scale）/地形装饰（沙丘/海面/岩壁）。
