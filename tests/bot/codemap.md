...
黑盒集成验收脚本目录。提供 `npm run bot` 命令执行的无头跑圈脚本，通过 `src/ai/simulate.ts` 驱动 bot 在全部内置赛道（`TRACK_DEFS` 9 条，含 canyon/alpine 夜间赛道）上完成全赛道矩阵跑圈，输出逐赛道 JSON 报告与整体通过/失败判定，作为里程碑门禁的验收层。自 M13 H8 引入全赛道矩阵回归以来，每里程碑（M14/M15/M16/M17/M18/M19/M20）验收均保持 9 条赛道全部完成、0 违规超标（可复现：赛道/车流由确定性随机种子生成）。
