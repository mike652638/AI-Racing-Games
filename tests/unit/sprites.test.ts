...
describe('路边景物生成', () => {
  ...
  test('按间隔成对放置，z 从间距一半开始（UX-10 + M27：树/仙人掌 ±1.55 / 路灯 ±1.8 外移路外）', () => {
    ...
    // 横向偏移按类型分层：树/仙人掌等 ±1.55，路灯 ±1.8（弯道路灯不再与路面相交，M27 修复弯道树位贴路面）
    expect(sprites.every((s) => Math.abs(s.offset) === (s.kind === 'lamp' ? 1.8 : 1.55))).toBe(true)
    for (const z of zs) {
      const pair = sprites.filter((s) => s.z === z)
      expect(pair).toHaveLength(2)
      const expectAbs = pair[0].kind === 'lamp' ? 1.8 : 1.55
